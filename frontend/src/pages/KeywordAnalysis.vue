<template>
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-8">
            <button @click="router.back()" class="transition-colors text-secondary-400 hover:text-secondary-600">
                <ArrowLeft class="w-5 h-5" />
            </button>
            <div>
                <h1 class="text-2xl font-bold text-secondary-900">Analisis Kata Kunci</h1>
                <p class="mt-1 text-sm text-secondary-500">Detail kecocokan CV kamu dengan deskripsi pekerjaan</p>
            </div>
        </div>
        ...
        <div v-else-if="!analysis" class="py-20 text-center">
            <SearchX class="w-16 h-16 mx-auto mb-4 text-secondary-300" />
            <p class="text-lg font-medium text-secondary-500">Analisis tidak tersedia</p>
            <p class="mt-1 mb-6 text-sm text-secondary-400">
        ...
        <div v-else class="flex flex-col gap-6">
            ...
            <div class="card">
                <div class="flex items-center gap-2 mb-6">
                    ...
                    <h2 class="font-semibold text-secondary-900">Skor Keseluruhan</h2>
                </div>
                ...
                <div class="flex items-center gap-8">
                    <div class="relative w-28 h-28 shrink-0">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <span class="text-2xl font-bold text-secondary-900">{{ analysis.overallMatch }}</span>
                    ...
                    <div class="flex flex-col flex-1 gap-3">
                        <div v-for="(score, key) in analysis.categoryScores" :key="key">
                            <div class="flex items-center justify-between mb-1">
                                <span class="text-sm capitalize text-secondary-600">{{ categoryLabel(key) }}</span>
                                <span class="text-sm font-semibold text-secondary-900">{{ score }}%</span>
                            </div>
                            <div class="h-2 overflow-hidden rounded-full bg-secondary-100">
            ...
            <div class="card">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <h2 class="font-semibold text-secondary-900">Rekomendasi Utama</h2>
                </div>
                <ul class="flex flex-col gap-3">
                    <li v-for="(rec, index) in analysis.topRecommendations" :key="index"
                        ...
                        <p class="text-sm text-secondary-700">{{ rec }}</p>
                    </li>
                </ul>
            </div>
            ...
            <div class="card">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="font-semibold text-secondary-900">Kata Kunci yang Kurang</h2>
                    </div>
                ...
                <div class="flex flex-col gap-2">
                    <div v-for="kw in sortedMissing" :key="kw.keyword"
                        class="flex items-start justify-between p-3 border border-red-100 rounded-xl bg-red-50">
                        <div class="flex items-start gap-3">
                            <span :class="importanceBadge(kw.importance)" class="mt-0.5 shrink-0">
                                {{ importanceLabel(kw.importance) }}
                            </span>
                            <div>
                                <p class="text-sm font-semibold text-secondary-900">{{ kw.keyword }}</p>
                                <p class="text-xs text-secondary-500 mt-0.5">{{ kw.suggestion }}</p>
                            </div>
                        </div>
            ...
            <div class="card">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="font-semibold text-secondary-900">Kata Kunci yang Ada</h2>
                    </div>
...
import {
    ArrowLeft, BarChart2, Lightbulb, XCircle,
    CheckCircle, SearchX
...
const sortedMissing = computed(() => {
    ...
    return [...analysis.value.missingKeywords].sort((a, b) =>
...
const importanceBadge = (importance) => ({
    high: 'badge bg-red-200 text-red-800',
    medium: 'badge bg-yellow-200 text-yellow-800',
    low: 'badge bg-secondary-200 text-secondary-700'
}[importance] || 'badge')
...
}[importance] || 'border-secondary-200 bg-secondary-50 text-secondary-600')
